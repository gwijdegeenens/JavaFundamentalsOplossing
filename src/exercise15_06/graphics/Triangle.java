...
public class Triangle extends Shape {
   ...
   public Triangle(int width, int height, int perpendicular, int x,
      int y) {
      super(x, y);
      setWidth(width);
      setHeight(height);
   ...
   public Triangle(Triangle tri) {
      this(tri.width, tri.height, tri.perpendicular, tri.getX(),
         tri.getY());
   }
   ...
   public double getPerimeter() {
      return width + Math.hypot(height, perpendicular) + Math
         .hypot(height, (width - perpendicular));
   }
